...
void IncreaseRing(){
    increaseSize();
    sem_post(&globalRing->semaphores->empty);
}

void DecreaseRing(){
    decreaseSize();
    sem_trywait(&globalRing->semaphores->empty);
}
...
void* produceCycle(void*) {
    while (1) {
        ...
        if (globalRing->tail < globalRing->size) {
            std::cout << "\n<Created message>\n";
            printData(data);
        }
        sleep(3);
...
void showStats() {
    sem_wait(&(globalRing->semaphores->mutex));
    ...
    sem_post(&(globalRing->semaphores->mutex));
}