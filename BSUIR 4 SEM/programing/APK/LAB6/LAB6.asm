...
DATA_BEGIN      = $
    ...
    GDT_BEGIN   = $
    GDT label   word                            
    GDT_0       S_DESC <0,0,0,0,0,0>                           ; 0,         
    GDT_GDT     S_DESC <GDT_SIZE-1,,,10010010b,0,>             ; 8,      
    GDT_CODE_RM S_DESC <SIZE_CODE_RM-1,,,10011010b,0,>         ; 16,    
    GDT_DATA    S_DESC <SIZE_DATA-1,,,11110010b,0,>      	   ; 24,  
    GDT_STACK   S_DESC <1000h-1,,,10010010b,0,>                ; 32,      
    GDT_TEXT    S_DESC <2000h-1,8000h,0Bh,11110010b,0,0> 	   ; 40,  (  )
    GDT_CODE_PM S_DESC <SIZE_CODE_PM-1,,,10011010b,01000000b,> ; 48,       
    GDT_IDT     S_DESC <SIZE_IDT-1,,,10010010b,0,>             ; 56,  
    ...
    MSG_TIME            db "RM will be in (s)",0
    MSG_COUNT           db "Count of interrupt:",0
    MSG_EXC             db "Exception: XX",0
    MSG_ENTER           db "Enter timer (ms): $"
    MSG_ERROR           db "E!R!R!O!R$"
...
; 	
FILL_IDT:                           
    irpc    N, 0123456789ABCDEF	;   0               
		lea eax, EXC_0&N			
        mov IDT_0&N.OFFS_L,ax
        shr eax, 16
        mov IDT_0&N.OFFS_H,ax
    endm
    irpc    N, 0123456789ABCDEF ;   1                 
        lea eax, EXC_1&N
    ...
    or  al,00000001b                ;              
    mov cr0,eax                     ;      
;  CS:IP :  OVERLOAD_SEGMENT_REGISTERS      	
OVERLOAD_CS:                                    
    db  0eah								;  far jmp                                    
...
EXIT:                                           
    ...
    call FILL_CR_0_BUFFER_RM    ;  CR0              
   ; lea dx, BUFFER_CR_0_RM		;   CR0
   ; mov ah, 9h
   ; int 21h
